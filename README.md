...
본 프로젝트는 KT내부에서 사용하는 물품 구매 플랫폼인 KT커머스를 MSA형태로 재구성한 프로젝트입니다.
모놀리식으로 구현된 플랫폼을 MSA로 설계해보며 서비스에 대한 아이디어보다는 강의 내용을 다시 복기하는게 목표입니다.
...
기능적 요구사항
1. 고객이 상품을 주문한다.
2. 상품을 주문함과 동시에 결제가 진행된다.
3. 고객이 주문을 취소할 수 있다.
4. 주문함과 동시에 재고가 감소된다.
5. 재고가 감소될 때 재고에 따라 재고 감소가 성공하거나 재고없음을 확인한다.
6. 재고가 없으면 주문이 취소된다.
7. 결제가 진행되면 결제 가능 여부를 체크한다.
8. 결제가 가능하면 고객 계좌 잔액을 업데이트한다.
9. 결제가 불가능하면 결제가 거절된다.
10. 결제가 거절되면 주문 상태가 변경된다.
11. 전체적인 상품에 대한 정보를 얻을 수 있다.
12. 재고가 감소하면 배송이 시작된다.
13. 주문이 취소되면 배송도 취소된다.
14. 주문에 대한 상태와 정보를 한 번에 볼 수 있어야한다.(viewpage)

비기능적 요구사항
1. 트랜잭션
    1. 결제가 되지 않은 예약 건은 성립되지 않아야 한다.  (Sync 호출)
1. 장애격리
    1. 재고 업데이트 및 배송 기능이 수행되지 않더라도 상품 주문은 365일 24시간 받을 수 있어야 한다  Async (event-driven), Eventual Consistency
1. 성능
    1. 모든 상품에 대한 정보를 확인할 수 있어야한다.  
    1. 주문에 대한 정보와 배송 상태를 확인할 수 있어야한다.(CQRS)
...
![image](https://private-user-images.githubusercontent.com/77838744/374408284-6154a1d8-0f15-441b-87d2-6219a34ced2a.png?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..5WHwVsvoW4IMfg9M4FP2ej6W7ZxxyTdmm0y0twFpleE)
